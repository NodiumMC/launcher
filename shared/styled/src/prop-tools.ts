...
type MatchObject<P, S> = S extends boolean
  ? BooleanMatch<P>
  : { [K in NonNullable<S> as any]?: StyleFn<P> }
...
export function variants<P = any, V = any>(
  selector: PropSelector<P, V>,
  match: MatchObject<P, V>,
): StyleFn<P> {
  return ((props: P) => {
...
export function ifProp<P = any, V = any>(
  selector: PropSelector<P, V>,
  then: StyleFn<P>,
  or?: StyleFn<P>,
) {
