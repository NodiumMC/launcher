...
const rulifyArgumnets = async (args: ArgumentsArray): Promise<ArgumentsArray> =>
  args.filterAsync(async arg => typeof arg === 'string' || await Rules(arg, ['has_custom_resolution']).then(r => r.allow))

const flatArguments = async (args: ArgumentsArray): Promise<string[]> => {
  const stringArgs = await args.mapAsync(async arg => typeof arg === 'string' ? [arg] : await Rules(arg).then(r => r.value ?? []))
  return stringArgs.flat()
}

const placeholderifyArguments = (options: VersionedLaunchOptions, classPath: string) => async (args: string[]): Promise<string[]> => {
  return args.mapAsync(async arg =>
    arg.replaceAll('${auth_player_name}', options.username)
      ...
      .replaceAll('${natives_directory}', await join(options.clientDir, 'natives'))
      ...
      .replaceAll('${library_directory}', await join(options.gameDataDir, 'libraries'))
...
export const compileArguments =
  async (options: VersionedLaunchOptions): Promise<string[]> => {
    const game = options.version.arguments.game ?? options.minecraftArgs
    const jvm = options.version.arguments.jvm ?? options.javaArgs
    const classPathString = await compileClasspath(options.version, options.gameDataDir, options.clientDir).then(v => v.join(delimiter))
    const gargs = await rulifyArgumnets(game).then(flatArguments).then(placeholderifyArguments(options, classPathString))
    const jargs = await rulifyArgumnets(jvm).then(flatArguments).then(placeholderifyArguments(options, classPathString))
